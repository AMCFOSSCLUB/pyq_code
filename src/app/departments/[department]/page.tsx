...
const CURRENT_CURRICULUM_YEAR = 2023;
...
export default function DepartmentPage({ params }: DepartmentPageProps) {
  ...
  const [newSubject, setNewSubject] = useState<SubjectFormState>({
    ...
    typicalSemester: "",
    note: "",
    curriculumYear: String(CURRENT_CURRICULUM_YEAR),
  });
  ...
  // Only true once the server confirms this visitor is a whitelisted admin.
  // Defaults to false, so edit/delete controls stay hidden for everyone
  // until (and unless) this check comes back positive — the server-side
  // routes enforce this too, this is just for the UI.
  ...
  useEffect(() => {
    fetch('/api/admin/me')
      .then((res) => res.json())
      .then((data) => setIsAdmin(!!data?.isAuthorizedAdmin))
  ...
  function openAddModal() {
    ...
    setNewSubject({ code: "", name: "", typicalSemester: "", note: "", curriculumYear: String(CURRENT_CURRICULUM_YEAR) });
  ...
  function openEditModal(subject: Subject) {
    ...
    setNewSubject({
      ...
      curriculumYear: String((subject as any).curriculumYear ?? CURRENT_CURRICULUM_YEAR),
    });
    setShowAddSubject(true);
  }
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          curriculumYear: newSubject.curriculumYear
            ? Number(newSubject.curriculumYear)
  ...
  const availableCurriculumYears: number[] = [];
  ...
  // Unique semesters available within the active curriculum
  ...
  return (
    <div className="container page-top section-sm">
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
              Showing {filteredSubjects.length} of {curriculumSubjects.length} subjects
            </div>
          </div>
        </div>
      ...
      {/* Subjects List (flat — semester pills above are a filter, not a grouping) */}
      <div style={{ maxWidth: '1100px' }}>
        {filteredSubjects.length === 0 ? (
          ...
          <SubjectList
            ...
            onDelete={isAdmin ? deleteSubject : undefined}
          />
        )}
      </div>
      ...
      {showAddSubject && (
        ...
        >
          ...
          >
            ...
            <input
              placeholder="Curriculum Year (e.g. 2023, 2027)"
              value={newSubject.curriculumYear}
              onChange={(e) =>
                setNewSubject({
                  ...newSubject,
                  curriculumYear: e.target.value,
                })
              }
              style={inputStyle}
            />
