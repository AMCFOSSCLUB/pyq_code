...
export async function POST(req: NextRequest) {
  ...
  const { subjectCode, courseTitle, filename, content } = await req.json();
  ...
  try {
    // Commit to GitHub
    const commitSha = await uploadFile(courseTitle?.trim() || 'UNKNOWN_SUBJECT', code, filename, content, uploaderGitHub);

    // Write Firestore upload doc
    const uploadRef = await db.collection('uploads').add({
      subjectTitle: courseTitle?.trim() || 'UNKNOWN_SUBJECT',
      subjectCode: code,
      filename,
      uploaderGitHub,
      uploaderProfileUrl,
    ...
    await db.collection('auditLogs').add({
      ...
      targetFile: `${courseTitle?.trim() || 'UNKNOWN_SUBJECT'}/${code}/${filename}`,
