...
export async function GET(
  ...
  try {
    ...
    db.collection('auditLogs').add({
      ...
      targetFile: `${data.subjectTitle || 'UNKNOWN_SUBJECT'}/${data.subjectCode}/${data.filename}`,
      timestamp: FieldValue.serverTimestamp(),
    }).catch(() => {});

    const url = getFileDownloadUrl(data.subjectTitle || 'UNKNOWN_SUBJECT', data.subjectCode, data.filename);
