      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
        ...
        <div className="glass-card" style={{ padding: 24 }}>
          <h3 style={{ fontSize: '1rem', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <BookOpen size={16} style={{ color: 'var(--violet-light)' }} /> Top Subjects by Papers
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {topSubjects.map(([code, count]) => (
              <div key={code}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <code style={{ fontSize: '0.82rem', color: 'var(--violet-light)', fontFamily: 'JetBrains Mono, monospace' }}>{code}</code>
        ...
        <div className="glass-card" style={{ padding: 24 }}>
          <h3 style={{ fontSize: '1rem', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <TrendingUp size={16} style={{ color: 'var(--cyan-light)' }} /> Top Subjects by Downloads
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {topDownloaded.map(([code, count]) => (
              <div key={code}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <code style={{ fontSize: '0.82rem', color: 'var(--cyan-light)', fontFamily: 'JetBrains Mono, monospace' }}>{code}</code>
                  <span style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>{count.toLocaleString()}</span>
                </div>
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${(count / maxDownloads) * 100}%`, background: 'linear-gradient(90deg, #06b6d4, #67e8f9)' }} />
                </div>
              </div>
            ))}
        ...
        <div className="glass-card" style={{ padding: 24, gridColumn: '1 / -1' }}>
          <h3 style={{ fontSize: '1rem', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Users size={16} style={{ color: '#f9a8d4' }} /> Top Contributors
          </h3>
