...
export function VisualEffects() {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      ...
      const dustCount = window.innerWidth < 768 ? 20 : 45;
      particles = [];
      dust = [];
    ...
    const handleResize = () => {
      ...
      initParticles();
      centerGrad = ctx.createRadialGradient(width / 2, height / 2, 50, width / 2, height / 2, Math.max(width, height) * 0.7);
    ...
    let centerGrad = ctx.createRadialGradient(width / 2, height / 2, 50, width / 2, height / 2, Math.max(width, height) * 0.7);
    centerGrad.addColorStop(0, 'rgba(18, 205, 0, 0.045)');
    centerGrad.addColorStop(0.5, 'rgba(18, 205, 0, 0.015)');
    centerGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
    ...
    const animate = () => {
      ...
      // 1. Draw soft glowing center-screen green nebula (gradient cached, not rebuilt every frame)
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distSq >= connectionDist * connectionDist) continue; // skip sqrt for far-apart pairs
          ...
          let opacity = (1 - dist / connectionDist) * 0.02; // Duller base subtle connections
            ...
            let mouseDist2 = 1000;
            if (mouseX !== -1000 && mouseY !== -1000) {
              const md1Sq = (mouseX - p1.x) ** 2 + (mouseY - p1.y) ** 2;
              const md2Sq = (mouseX - p2.x) ** 2 + (mouseY - p2.y) ** 2;
              // only take the sqrt if within the 250px radius that actually matters below
              mouseDist1 = md1Sq < 62500 ? Math.sqrt(md1Sq) : 1000;
              mouseDist2 = md2Sq < 62500 ? Math.sqrt(md2Sq) : 1000;
            }
